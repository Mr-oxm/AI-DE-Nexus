...
async function getCheatsheetHubs() {
  try {
    const base = process.env.NEXTAUTH_URL || "http://localhost:3000";
    const cookieStore = await cookies();
    const res = await fetch(`${base}/api/cheatsheets/hubs`, {
...
export default async function CheatsheetsHubPage() {
  const hubs = await getCheatsheetHubs();
    ...
    <div className="px-6 md:px-10 py-12 max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-2 duration-400">
      <div className="mb-12">
        ...
        <p className="text-sm text-zinc-500 max-w-xl leading-relaxed">
          Quick reference cheatsheets — code snippets and commands at a glance.
        </p>
      </div>
      ...
      <div className="mb-5">
        ...
        {hubs.length === 0 ? (
          <div className="p-8 rounded-xl border border-zinc-800 bg-zinc-900/30 text-center text-zinc-500">
            <p className="text-sm">No cheatsheets yet.</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {hubs.map((hub: { _id: string; title: string; description: string; forkedFrom?: { title: string }; starCount?: number; starred?: boolean }) => (
              <HubCard key={hub._id} type="cheatsheet" hub={hub} initialStarred={hub.starred} />
            ))}
          </div>
        )}
      </div>
