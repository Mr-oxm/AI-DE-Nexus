...
export async function GET(req: Request) {
  try {
    ...
    let hubs = await CheatsheetHub.find()
    ...
    const [counts, starredIds] = await Promise.all([
      hubObjectIds.length > 0
        ? Star.aggregate([
            { $match: { targetType: "cheatsheetHub", targetId: { $in: hubObjectIds } } },
    ...
    console.error(e);
    return NextResponse.json({ error: "Failed to fetch cheatsheet hubs" }, { status: 500 });
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    await connectDB();
    const hub = await CheatsheetHub.create({
      title,
    ...
    console.error(e);
    return NextResponse.json({ error: "Failed to create cheatsheet hub" }, { status: 500 });
  }
}
