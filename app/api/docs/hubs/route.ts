  ...
  try {
    ...
    const [counts, starredIds] = await Promise.all([
      hubObjectIds.length > 0
        ? Star.aggregate([
            { $match: { targetType: "docHub", targetId: { $in: hubObjectIds } } },
    ...
    console.error(e);
    return NextResponse.json({ error: "Failed to fetch doc hubs" }, { status: 500 });
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    console.error(e);
    return NextResponse.json({ error: "Failed to create doc hub" }, { status: 500 });
  }
}
